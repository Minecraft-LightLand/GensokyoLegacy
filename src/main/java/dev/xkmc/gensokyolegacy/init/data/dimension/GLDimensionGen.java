...
public class GLDimensionGen {
	...
	public static final ResourceKey<NormalNoise.NoiseParameters> NP_SIMPLE_A = ResourceKey.create(Registries.NOISE, GensokyoLegacy.loc("simple_a"));
	public static final ResourceKey<NormalNoise.NoiseParameters> NP_SIMPLE_B = ResourceKey.create(Registries.NOISE, GensokyoLegacy.loc("simple_b"));
	public static final ResourceKey<NormalNoise.NoiseParameters> NP_SIMPLE_0 = ResourceKey.create(Registries.NOISE, GensokyoLegacy.loc("simple_0"));
	...
	public static void init(DataProviderInitializer init) {
		...
		init.add(Registries.NOISE, (ctx) -> {
			ctx.register(NP_SIMPLE_A, new NormalNoise.NoiseParameters(-7, DoubleList.of(1, 0.7, 0.5)));
			ctx.register(NP_SIMPLE_B, new NormalNoise.NoiseParameters(-7, DoubleList.of(1, 0.7, 0.5)));
			ctx.register(NP_SIMPLE_0, new NormalNoise.NoiseParameters(-6, DoubleList.of(1, 0.7, 0.5)));
		});
		...
		init.add(Registries.LEVEL_STEM, (ctx) -> {
			...
			var climate = new Climate.ParameterList<Holder<Biome>>(List.of(
					Pair.of(point(1f, -1f, 0f), biome.get(GLBiomeGen.BIOME_MAINLAND).orElseThrow()),
					Pair.of(point(-1f, 1f, 0f), biome.get(GLBiomeGen.BIOME_EDGE).orElseThrow()),
					Pair.of(point(1f, 1f, 0f), biome.get(GLBiomeGen.BIOME_ISLAND).orElseThrow()),
					Pair.of(point(-1f, -1f, 0f), biome.get(GLBiomeGen.BIOME_VOID).orElseThrow())
			));
	...
	private static Climate.ParameterPoint point(float t0, float v0, float offset) {
		return Climate.parameters(
				Climate.Parameter.point(v0),
				Climate.Parameter.point(t0),
