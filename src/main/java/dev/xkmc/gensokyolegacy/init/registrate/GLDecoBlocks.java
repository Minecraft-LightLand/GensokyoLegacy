...
public class GLDecoBlocks {
	...
	static {
		TAB = GensokyoLegacy.REGISTRATE.buildModCreativeTab("building_blocks", "Gensokyo Legacy - Building Blocks",
				e -> e.icon(GLDecoBlocks.ICE_SET.block::asStack));
	...
	public static class BrickSet {
		...
		public BrickSet(String id, BlockBehaviour.Properties prop) {
			...
			stairs = GensokyoLegacy.REGISTRATE.block(id + "_brick_stairs", p ->
					...
					.blockstate((ctx, pvd) -> pvd.stairsBlock(ctx.get(), id + "_brick", side))
					.tag(BlockTags.MINEABLE_WITH_PICKAXE, BlockTags.STAIRS)
					.item().tag(ItemTags.STAIRS).build()
					.register();
			slab = GensokyoLegacy.REGISTRATE.block(id + "_brick_slab", p ->
							new SlabBlock(prop))
					...
					.register();
			vertical = GensokyoLegacy.REGISTRATE.block(id + "_brick_vertical_slab", p ->
					...
					.tag(BlockTags.MINEABLE_WITH_PICKAXE).item().build().register();
		}
