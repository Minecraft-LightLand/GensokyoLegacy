...
public record StructSimpleBuilding(
	...
	@Override
	public void registerTemplatePools(BootstrapContext<StructureTemplatePool> ctx, Identifier id) {
		var empty = ctx.lookup(Registries.TEMPLATE_POOL)
				.getOrThrow(ResourceKey.create(Registries.TEMPLATE_POOL, Identifier.withDefaultNamespace("empty")));
	...
	@Override
	public void registerProcessors(BootstrapContext<StructureProcessorList> ctx, Identifier id) {
		ctx.register(ResourceKey.create(Registries.PROCESSOR_LIST, id),
				new StructureProcessorList(processors()));
	}

	@Override
	public void registerStructure(BootstrapContext<Structure> ctx, Identifier id, HolderSet.Named<Biome> biome) {
